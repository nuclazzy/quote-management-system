-- ========================================
-- 견적 관리 시스템 성능 최적화 인덱스 (수정된 버전)
-- 05_indexes_fixed.sql
-- ========================================

-- 주의: CREATE INDEX CONCURRENTLY는 트랜잭션 블록 내에서 실행할 수 없으므로
-- 일반 CREATE INDEX로 변경합니다.
...
-- 한국어 전문 검색 인덱스
CREATE INDEX IF NOT EXISTS idx_customers_name_gin ON customers USING gin(to_tsvector('korean', name));

-- 견적서 프로젝트 제목 검색 인덱스
CREATE INDEX IF NOT EXISTS idx_quotes_project_title_gin ON quotes USING gin(to_tsvector('korean', project_title));

-- 마스터 아이템 이름 검색 인덱스
CREATE INDEX IF NOT EXISTS idx_master_items_name_gin ON master_items USING gin(to_tsvector('korean', name));
...
-- 느린 쿼리 모니터링을 위한 설정 가이드
DO $$
BEGIN
    -- 느린 쿼리 로깅 활성화 (1초 이상 걸리는 쿼리)
    -- 실제 운영에서는 Supabase 대시보드에서 설정
    RAISE NOTICE '========================================';
    RAISE NOTICE '성능 모니터링 설정 가이드:';
    RAISE NOTICE '1. Supabase 대시보드에서 느린 쿼리 로깅 활성화';
    RAISE NOTICE '2. 정기적으로 index_usage_stats 뷰 확인';
    RAISE NOTICE '3. 사용되지 않는 인덱스는 제거 고려';
    RAISE NOTICE '4. table_performance_stats로 테이블 상태 모니터링';
    RAISE NOTICE '========================================';
END $$;
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ 기본 테이블 인덱스 (50개+)';
    RAISE NOTICE '✅ 전문 검색용 GIN 인덱스';
