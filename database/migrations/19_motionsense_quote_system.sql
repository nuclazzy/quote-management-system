-- 모션센스 맞춤형 견적서 시스템 데이터베이스 스키마
-- Migration: 19_motionsense_quote_system.sql
...
CREATE TRIGGER trigger_update_project_cost_expenses
    AFTER INSERT OR UPDATE OR DELETE ON project_expenses
    FOR EACH ROW
    EXECUTE FUNCTION update_project_total_cost();

CREATE TRIGGER trigger_update_project_cost_transactions
...
CREATE POLICY "Enable all for authenticated users" ON quote_items FOR ALL USING (auth.role() = 'authenticated');
...
('(주)영상제작소', '김감독', '02-1234-5678', 'director@video.co.kr'),
('사운드스튜디오', '박엔지니어', '02-2345-6789', 'sound@studio.co.kr'),
('조명장비렌탈', '이대표', '02-3456-7890', 'lighting@rental.co.kr')
...
COMMENT ON TABLE quotes IS '견적서 메인 테이블';
COMMENT ON TABLE quote_groups IS '견적서 내 그룹 (행사 진행, 영상 제작 등)';
COMMENT ON TABLE quote_items IS '견적서 내 항목 (촬영 서비스, 편집 작업 등)';
COMMENT ON TABLE quote_details IS '견적서 내 세부내용 (메인 카메라 촬영, 1차 편집 등) - 스냅샷 적용';
COMMENT ON TABLE suppliers IS '공급처/매입처 관리';
COMMENT ON TABLE master_items IS '마스터 품목 (세부내용 단위)';
COMMENT ON TABLE projects IS '프로젝트 (계약 완료된 견적서)';
COMMENT ON TABLE transactions IS '정산 관리 (공식 매입/매출)';
COMMENT ON TABLE project_expenses IS '프로젝트 기타 경비';

-- 완료 메시지
SELECT 'Motionsense Quote System Database Schema Created Successfully!' as status;