-- ========================================
-- 견적 관리 시스템 성능 최적화 인덱스 (최종 버전)
-- 05_indexes_final.sql
-- ========================================
...
-- ========================================
-- 5. 전문 검색용 인덱스 (영어 기본 설정)
-- ========================================

-- 텍스트 검색 인덱스 (기본 english 설정 사용)
CREATE INDEX IF NOT EXISTS idx_customers_name_gin ON customers USING gin(to_tsvector('english', name));
CREATE INDEX IF NOT EXISTS idx_quotes_project_title_gin ON quotes USING gin(to_tsvector('english', project_title));
CREATE INDEX IF NOT EXISTS idx_master_items_name_gin ON master_items USING gin(to_tsvector('english', name));

-- JSON 데이터 검색을 위한 GIN 인덱스
CREATE INDEX IF NOT EXISTS idx_quote_templates_data_gin ON quote_templates USING gin(template_data);

-- ========================================
-- 6. 대소문자 구분 없는 검색용 인덱스
-- ========================================

-- 대소문자 구분 없는 이름 검색 (한국어 지원)
...
CREATE INDEX IF NOT EXISTS idx_master_items_name_lower ON master_items(lower(name));
CREATE INDEX IF NOT EXISTS idx_quotes_title_lower ON quotes(lower(project_title));

-- ========================================
-- 7. 시간 기반 인덱스 (날짜 범위 검색 최적화)
-- ========================================
...
-- 견적서 총액 계산 최적화
CREATE INDEX IF NOT EXISTS idx_quote_totals_calc ON quote_details(quote_item_id, quantity, days, unit_price, is_service);
...
-- 월별 매출 리포트 최적화
CREATE INDEX IF NOT EXISTS idx_projects_monthly_revenue ON projects(
...
-- ========================================
-- 12. 한국어 검색을 위한 사용자 정의 함수
-- ========================================

-- 한국어 검색 헬퍼 함수 (띄어쓰기 무시)
CREATE OR REPLACE FUNCTION korean_search(text_column TEXT, search_term TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    -- 띄어쓰기와 특수문자를 제거하고 검색
    RETURN replace(lower(text_column), ' ', '') LIKE '%' || replace(lower(search_term), ' ', '') || '%';
END;
$$;

-- 한국어 검색용 인덱스 (함수 기반)
CREATE INDEX IF NOT EXISTS idx_customers_korean_search ON customers(replace(lower(name), ' ', ''));
CREATE INDEX IF NOT EXISTS idx_quotes_korean_search ON quotes(replace(lower(project_title), ' ', ''));
CREATE INDEX IF NOT EXISTS idx_master_items_korean_search ON master_items(replace(lower(name), ' ', ''));
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ 기본 테이블 인덱스 (60개+)';
    RAISE NOTICE '✅ 전문 검색용 인덱스 (영어 설정)';
    RAISE NOTICE '✅ 한국어 검색용 사용자 정의 함수';
