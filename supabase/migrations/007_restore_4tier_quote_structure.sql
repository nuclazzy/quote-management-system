-- 4단계 견적서 구조 복원 마이그레이션
-- quotes → quote_groups → quote_items → quote_details

-- 기존 2단계 구조 백업
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'quote_items_backup') THEN
    EXECUTE 'CREATE TABLE quote_items_backup AS SELECT * FROM quote_items';
  END IF;
END $$;

-- 1. quote_groups 테이블 생성
...
-- 2. quote_details 테이블 생성 (스냅샷 데이터 저장)
CREATE TABLE IF NOT EXISTS quote_details (
    ...
    -- 메타데이터
    master_item_id UUID REFERENCES master_items(id), -- 원본 참조 (스냅샷 목적)
    sort_order INTEGER DEFAULT 0,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 3. quote_items 테이블 구조 변경 (quote_groups 참조하도록)
-- 기존 quote_id 참조를 quote_group_id로 변경
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM quote_items LIMIT 1) THEN
        ...
        LOOP
            ...
            FOR item_rec IN 
                SELECT * FROM quote_items WHERE quote_group_id = group_id
            LOOP
                -- 기존 item 데이터를 detail로 이전 (존재하는 컬럼만 사용)
                INSERT INTO quote_details (
                    ...
                    item_rec.id,
                    COALESCE(item_rec.name, '품목'),
                    COALESCE(item_rec.description, ''),
                    COALESCE(item_rec.quantity, 1),
                    COALESCE(item_rec.days, 1),
                    COALESCE(item_rec.unit, '개'),
                    COALESCE(item_rec.unit_price, 0),
                    COALESCE(item_rec.cost_price, 0),
                    COALESCE(item_rec.sort_order, 0)
                );
            END LOOP;
        END LOOP;
    END IF;
...
-- 5. quote_items 테이블 정리
-- 의존성이 있는 객체들을 먼저 처리하고 불필요한 컬럼들 제거
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quote_items' AND column_name = 'item_name') THEN
        ALTER TABLE quote_items DROP COLUMN item_name;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quote_items' AND column_name = 'item_description') THEN
        ALTER TABLE quote_items DROP COLUMN item_description;
    END IF;
...
-- quote_group_id를 NOT NULL로 변경 (데이터가 있는 경우에만)
DO $$
BEGIN
...
-- 10. quote_items RLS 정책 재생성 (새로운 구조에 맞게)
CREATE POLICY IF NOT EXISTS "quote_items_select" ON quote_items FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "quote_items_insert" ON quote_items FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "quote_items_update" ON quote_items FOR UPDATE USING (true);
CREATE POLICY IF NOT EXISTS "quote_items_delete" ON quote_items FOR DELETE USING (true);

-- 11. quote_summary 뷰 재생성 (4단계 구조에 맞게)
