...
export class NotificationService {
  ...
  static async notifyQuoteCreated(
    quoteId: string,
    createdBy: string
  ): Promise<void> {
    try {
  ...
  static async notifyQuoteStatusChanged(
    ...
    try {
      ...
      const targetUsers = Array.from(
        new Set([quote.created_by, ...adminUsers])
  ...
  static async notifyProjectStatusChanged(
    ...
    try {
      ...
      const adminUsers = await this.getAdminUsers();
      const targetUsers = Array.from(
        new Set([project.created_by, ...adminUsers])
      ).filter((id) => id && id !== updatedBy);
  ...
  static async notifySettlementDue(): Promise<void> {
    try {
      ...
      for (const transaction of transactions) {
        const targetUsers = Array.from(
          new Set([transaction.created_by, ...adminUsers])
        ).filter((id) => id);
  ...
  static async notifySettlementCompleted(
  ...
  static async notifyPermissionChanged(
    userId: string,
    newRole: string,
    changedBy: string
  ): Promise<void> {
    try {
      ...
      const adminUsers = await this.getAdminUsers();
      const targetUsers = Array.from(new Set([userId, ...adminUsers])).filter(
        (id) => id && id !== changedBy
      );
