...
export class QuoteService {
  ...
  static async getQuotes(filter?: QuoteFilter, page = 1, perPage = 20) {
    ...
    if (filter) {
      ...
      if (filter.search) {
        query = query.or(
          `project_title.ilike.%${filter.search}%,quote_number.ilike.%${filter.search}%,customer_name_snapshot.ilike.%${filter.search}%`
        );
      }
   ...
   */
  private static async createQuoteStructure(
    quoteId: string,
  ...
  static async updateQuoteTotal(quoteId: string) {
    try {
      ...
      const { data, error } = await supabase.rpc('calculate_quote_total', {
