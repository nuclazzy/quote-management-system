...
// 여러 권한 동시 체크
export async function checkMultiplePermissions(permissionNames: string[]): Promise<Record<string, boolean>> {
...
// 역할 기반 접근 제어
export async function hasRole(role: 'super_admin' | 'admin' | 'member'): Promise<boolean> {
  try {
    const { data: { user }, error: authError } = await supabase.auth.getUser()
...
// 최소 역할 요구사항 체크
export async function hasMinimumRole(minimumRole: 'member' | 'admin' | 'super_admin'): Promise<boolean> {
  try {
    const { data: { user }, error: authError } = await supabase.auth.getUser()
