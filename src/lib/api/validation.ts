...
export function validateQuote(data: any): QuoteValidation {
  ...
  const quote_groups = data.quote_groups.map((group: any, groupIndex: number) => {
    ...
    if (typeof group.sort_order !== 'number') {
      throw new Error(`Quote group ${groupIndex + 1}: Sort order must be a number`)
    }
    
    if (!Array.isArray(group.quote_items)) {
      throw new Error(`Quote group ${groupIndex + 1}: Quote items must be an array`)
    }
    
    // Validate quote items
    const quote_items = group.quote_items.map((item: any, itemIndex: number) => {
      if (!item.item_name || typeof item.item_name !== 'string') {
        throw new Error(`Quote group ${groupIndex + 1}, item ${itemIndex + 1}: Item name is required`)
      }
      
      if (typeof item.quantity !== 'number' || item.quantity <= 0) {
        throw new Error(`Quote group ${groupIndex + 1}, item ${itemIndex + 1}: Quantity must be a positive number`)
      }
      
      if (typeof item.unit_price !== 'number' || item.unit_price < 0) {
        throw new Error(`Quote group ${groupIndex + 1}, item ${itemIndex + 1}: Unit price must be a non-negative number`)
      }
      
      if (!Array.isArray(item.quote_item_details)) {
        throw new Error(`Quote group ${groupIndex + 1}, item ${itemIndex + 1}: Quote item details must be an array`)
      }
      
      // Validate quote item details
      const quote_item_details = item.quote_item_details.map((detail: any, detailIndex: number) => {
        if (!detail.detail_name || typeof detail.detail_name !== 'string') {
          throw new Error(`Quote group ${groupIndex + 1}, item ${itemIndex + 1}, detail ${detailIndex + 1}: Detail name is required`)
        }
        
        if (typeof detail.quantity !== 'number' || detail.quantity <= 0) {
          throw new Error(`Quote group ${groupIndex + 1}, item ${itemIndex + 1}, detail ${detailIndex + 1}: Quantity must be a positive number`)
        }
        
        if (typeof detail.unit_price !== 'number' || detail.unit_price < 0) {
          throw new Error(`Quote group ${groupIndex + 1}, item ${itemIndex + 1}, detail ${detailIndex + 1}: Unit price must be a non-negative number`)
        }
...
export function validateProject(data: any): ProjectValidation {
  ...
  if (data.status && !['planning', 'active', 'completed', 'cancelled'].includes(data.status)) {
    throw new Error('Status must be one of: planning, active, completed, cancelled')
  }
