...
export function useQuoteForm(initialData?: Partial<QuoteFormData>) {
  ...
  const updateItem = useCallback(
    (groupIndex: number, itemIndex: number, updates: any) => {
      const newGroups = [...formData.groups];
  ...
  const removeDetail = useCallback(
    (groupIndex: number, itemIndex: number, detailIndex: number) => {
      const newGroups = [...formData.groups];
      newGroups[groupIndex].items[itemIndex].details = newGroups[
        groupIndex
      ].items[itemIndex].details.filter((_, index) => index !== detailIndex);
      updateFormData({ groups: newGroups });
    },
    [formData.groups, updateFormData]
  );
