...
export const businessValidationRules = {
  ...
  customer: {
    ...
    phone: {
      ...
      custom: (value: string) => {
        if (value && !/^(\+82|0)([0-9]{1,2})-?([0-9]{3,4})-?([0-9]{4})$/.test(value)) {
...
// 객체 검증
export function validateObject(data: any, rules: ValidationRules): ValidationResult {
...
export const businessLogicValidators = {
  ...
  validateQuoteTotal: (items: any[], taxRate: number = 0) => {
    const subtotal = items.reduce((sum, item) => {
      return sum + (item.quantity * item.unit_price * (1 - (item.discount_rate || 0) / 100))
    }, 0)
  ...
  // 재고 검증
  validateStock: (requestedQuantity: number, availableStock: number, minStock: number = 0) => {
