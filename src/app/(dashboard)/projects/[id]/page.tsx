    ...
    <Box>
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              ...
              <Typography variant="body2" gutterBottom>
                <strong>상태:</strong> <Chip size="small" label={getStatusText(project.status)} color={getStatusColor(project.status)} />
              </Typography>
              {project.start_date && (
                <Typography variant="body2" gutterBottom>
                  <strong>기간:</strong> {project.start_date} ~ {project.end_date || '미정'}
                </Typography>
              )}
            </CardContent>
          </Card>
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              ...
              <Box display="flex" justifyContent="space-between" borderTop={1} borderColor="divider" pt={1}>
                <Typography variant="body2" fontWeight="bold">실제 수익:</Typography>
                <Typography 
                  variant="body2" 
                  fontWeight="bold" 
      ...
      <Dialog open={addTransactionDialog} onClose={() => setAddTransactionDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e: SelectChangeEvent<'income' | 'expense'>) => 
                  setNewTransaction(prev => ({ ...prev, type: e.target.value as 'income' | 'expense' }))
                }
              >
            ...
            <TextField
              ...
              onChange={(e) => setNewTransaction(prev => ({ ...prev, partner_name: e.target.value }))}
            />
            <TextField
              fullWidth
              label="항목명"
              value={newTransaction.item_name}
              onChange={(e) => setNewTransaction(prev => ({ ...prev, item_name: e.target.value }))}
            />
            <TextField
              fullWidth
              label="금액"
        ...
        <DialogActions>
          ...
          <Button onClick={handleAddTransaction} variant="contained">추가</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={addExpenseDialog} onClose={() => setAddExpenseDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button onClick={handleAddExpense} variant="contained">추가</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
