...
export default function ProjectsPage() {
  ...
  const [selectedProject, setSelectedProject] = useState<ProjectWithProfitability | null>(null);
  ...
  const fetchProjects = async () => {
    try {
      ...
      const projectsWithProfitability = data.map(project => ({
        ...
        profit_margin: project.total_revenue > 0 
          ? ((project.total_revenue - project.total_cost) / project.total_revenue) * 100 
          : 0
      }));
  ...
  const handleMenuOpen = (event: React.MouseEvent<HTMLElement>, project: ProjectWithProfitability) => {
              ...
              <CardContent>
                ...
                <Box mt={2}>
                  <Typography variant="body2">
                    총 매출: <strong>{project.total_revenue.toLocaleString()}원</strong>
                  </Typography>
                  <Typography variant="body2">
                    총 원가: <strong>{project.total_cost.toLocaleString()}원</strong>
                  </Typography>
                  <Typography 
                    variant="body2" 
                    color={project.net_profit >= 0 ? 'success.main' : 'error.main'}
                  >
                    순이익: <strong>{project.net_profit.toLocaleString()}원</strong>
                  </Typography>
                  <Typography 
                    variant="body2" 
                    color={project.profit_margin >= 0 ? 'success.main' : 'error.main'}
                  >
                    수익률: <strong>{project.profit_margin.toFixed(1)}%</strong>
                  </Typography>
                </Box>
