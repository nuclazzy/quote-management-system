...
export default function QuoteDetailPage({ params }: QuoteDetailPageProps) {
  ...
  useEffect(() => {
    const loadQuote = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : '견적서를 불러올 수 없습니다.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleConvertToProject = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/quotes/${quote.id}/convert-to-project`, {
        method: 'POST',
        headers: {
      ...
      setError(err instanceof Error ? err.message : '프로젝트 전환에 실패했습니다.')
    } finally {
        ...
        <Grid item xs={12} md={8}>
          <Paper sx={{ p: 3, mb: 3 }}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <Typography variant="body2" color="textSecondary">고객사</Typography>
                <Typography variant="body1">{quote.customer_name_snapshot}</Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography variant="body2" color="textSecondary">발행일</Typography>
              ...
              <Grid item xs={12} md={6}>
                <Typography variant="body2" color="textSecondary">VAT 처리</Typography>
                <Typography variant="body1">
                  {quote.vat_type === 'exclusive' ? 'VAT 별도' : 'VAT 포함'}
                </Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography variant="body2" color="textSecondary">대행수수료율</Typography>
                <Typography variant="body1">{quote.agency_fee_rate}%</Typography>
              </Grid>
              {quote.notes && (
                <Grid item xs={12}>
          ...
          <Paper sx={{ p: 3 }}>
            ...
            {quote.groups.map((group, groupIndex) => (
              <Box key={group.id} sx={{ mb: 4 }}>
                ...
                {group.items.map((item) => (
                  <Box key={item.id} sx={{ mb: 3, ml: 2 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                      ...
                      <Chip
                        label={item.include_in_fee ? '수수료 포함' : '수수료 제외'}
                        size="small"
                        variant="outlined"
                      />
                    </Box>
                ...
                {groupIndex < quote.groups.length - 1 && <Divider sx={{ my: 3 }} />}
              </Box>
            ))}
          </Paper>
        </Grid>
