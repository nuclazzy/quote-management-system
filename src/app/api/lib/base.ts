...
export function parseSearchParams(request: NextRequest) {
  ...
  // 정렬 파라미터 (화이트리스트 검증)
  const allowedSortFields = ['created_at', 'updated_at', 'name', 'title', 'status', 'total']
 ...
 * 안전한 필터 파라미터 추출
 */
export function extractFilterParams(request: NextRequest, allowedFilters: string[]) {
  const searchParams = request.nextUrl.searchParams
...
export async function callRPC<T>(
  ...
  if (error) {
    ...
    throw new Error(`데이터베이스 함수 실행 중 오류가 발생했습니다: ${error.message}`)
  }
...
export const ApiErrors = {
  NotFound: (resource: string) => 
    createErrorResponse('NOT_FOUND', `${resource}을(를) 찾을 수 없습니다.`, null, 404),
