...
export const CustomerSchema = z.object({
  name: z
    .string()
    .min(1, '고객사명을 입력해주세요.')
    .max(100, '고객사명은 100자를 초과할 수 없습니다.'),
  contact_person: z
    .string()
  ...
  phone: z
    .string()
    .regex(/^[0-9-+\s()]+$/, '유효한 전화번호 형식이어야 합니다.')
...
export const SupplierSchema = z.object({
  name: z
    .string()
    .min(1, '공급업체명을 입력해주세요.')
    .max(100, '공급업체명은 100자를 초과할 수 없습니다.'),
  contact_person: z
    .string()
    .max(50, '담당자명은 50자를 초과할 수 없습니다.')
    .optional(),
...
export const MasterItemSchema = z.object({
  ...
  default_unit_price: z
    .number()
    .min(0, '기본 단가는 0 이상이어야 합니다.')
    .default(0),
  default_unit: z
    .string()
    .min(1, '기본 단위를 입력해주세요.')
    .max(20, '단위는 20자를 초과할 수 없습니다.')
    .default('개'),
  is_active: z.boolean().default(true),
});
...
export const ProjectSchema = z.object({
  name: z
    .string()
    .min(1, '프로젝트명을 입력해주세요.')
    .max(100, '프로젝트명은 100자를 초과할 수 없습니다.'),
  quote_id: z.string().uuid('유효한 견적서를 선택해주세요.'),
  status: z
    .enum(['active', 'completed', 'on_hold', 'canceled'])
    .default('active'),
  parent_project_id: z.string().uuid().optional(),
  start_date: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, '유효한 날짜 형식이어야 합니다 (YYYY-MM-DD).')
    .optional(),
  end_date: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, '유효한 날짜 형식이어야 합니다 (YYYY-MM-DD).')
...
export const TransactionSchema = z.object({
  ...
  partner_name: z
    .string()
    .min(1, '거래처명을 입력해주세요.')
    .max(100, '거래처명은 100자를 초과할 수 없습니다.'),
  item_name: z
    .string()
    .min(1, '거래 항목명을 입력해주세요.')
    .max(100, '항목명은 100자를 초과할 수 없습니다.'),
  ...
  due_date: z
    ...
    .optional(),
  status: z
    .enum(['pending', 'processing', 'completed', 'issue'])
    .default('pending'),
  tax_invoice_status: z
...
export const ProjectExpenseSchema = z.object({
  ...
  description: z
    .string()
    .min(1, '경비 내용을 입력해주세요.')
    .max(200, '경비 내용은 200자를 초과할 수 없습니다.'),
...
export const BulkActionSchema = z.object({
  ...
  ids: z
    .array(z.string().uuid())
    .min(1, '최소 하나의 항목을 선택해주세요.')
    .max(100, '한 번에 최대 100개 항목까지 처리할 수 있습니다.'),
});
