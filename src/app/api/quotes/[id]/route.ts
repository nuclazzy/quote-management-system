...
import { withAuth, createApiResponse, createApiError, validateRequestBody } from '@/lib/api/base'
...
export async function PUT(
  ...
  return withAuth(request, async ({ user, supabase }) => {
    try {
      ...
      // 견적서 총액 계산
      const calculations = calculateQuoteTotals(validatedData.quote_groups, validatedData.tax_rate)
      ...
      // 기존 견적서 구성 요소들 삭제
      await supabase.from('quote_item_details').delete().in(
        'quote_item_id',
  ...
  return withAuth(request, async ({ user, supabase }) => {
    try {
      ...
      if (existingQuote.status !== status) {
        NotificationService.notifyQuoteStatusChanged(params.id, status, user.id).catch(error => {
          console.error('Failed to send quote status changed notification:', error)
        })
      }

      return createApiResponse(updatedQuote, 'Quote status updated successfully')
    } catch (error) {
