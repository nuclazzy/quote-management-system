...
async function handlePatch(req: AuthenticatedRequest, validatedData: any) {
  ...
  try {
    ...
    if (req.user.role === 'member' && existingQuote.created_by !== req.user.id) {
    ...
    if (!isValidStatusTransition(existingQuote.status, status)) {
      throw new ForbiddenError(`${existingQuote.status}에서 ${status}로 상태 변경이 불가능합니다.`)
    }
    ...
    if (error || !data || data.length === 0 || !data[0].success) {
      const errorMessage = data && data.length > 0 ? data[0].message : error.message
