...
import { BusinessError, ValidationError } from '../lib/middleware/error-handler'
  ...
  return withAuth(request, async ({ user, supabase }) => {
    ...
    if (filters.customer_id) {
      try {
        ...
        throw new ValidationError('잘못된 고객 ID 형식입니다.', { customer_id: filters.customer_id })
      }
    }
    ...
    if (filters.search) {
      ...
      // 이스케이프된 검색어 사용
      query = query.or(`title.ilike.%${searchTerm}%,quote_number.ilike.%${searchTerm}%`)
    }
    ...
    if (error) {
      ...
      throw new BusinessError('견적서 목록 조회 중 오류가 발생했습니다.', 500, 'FETCH_QUOTES_ERROR')
    }
...
export async function POST(request: NextRequest) {
  return withAuth(request, async ({ user, supabase }) => {
    ...
    if (!result || result.length === 0) {
      throw new BusinessError('견적서 생성 결과를 받을 수 없습니다.', 500, 'RPC_NO_RESULT')
    }
    
    const [createResult] = result
    
    if (!createResult.success) {
      throw new BusinessError(createResult.message || '견적서 생성에 실패했습니다.', 400, 'QUOTE_CREATION_FAILED')
    }
    ...
    if (fetchError || !createdQuote) {
      ...
      }, '견적서가 생성되었으나 상세 정보 조회에 실패했습니다.', 201)
    }
    
    // 알림 생성 (백그라운드 실행)
    sendQuoteCreatedNotification(createResult.quote_id, user.id).catch(error => {
...
function validateQuoteData(data: any) {
  ...
  for (const field of requiredFields) {
    if (!data[field]) {
      throw new ValidationError(`${field}는 필수 필드입니다.`, { missing_field: field })
    }
  }
  ...
  const validatedData = {
    ...
    project_id: data.project_id ? validateUUID(data.project_id, '프로젝트 ID') : null,
    tax_rate: data.tax_rate ? validateNumber(data.tax_rate, '세율', { min: 0, max: 100 }) : 10,
...
function validateQuoteGroups(groups: any[]) {
  if (!Array.isArray(groups) || groups.length === 0) {
    throw new ValidationError('최소 하나의 견적서 그룹이 필요합니다.', { groups })
  }

  return groups.map((group, groupIndex) => {
    if (!group.title) {
      throw new ValidationError(`그룹 ${groupIndex + 1}의 제목이 필요합니다.`, { group_index: groupIndex })
    }

    const validatedGroup = {
      title: sanitizeString(group.title, 200),
      sort_order: validateNumber(group.sort_order || 0, '정렬 순서', { integer: true, min: 0 }),
      quote_items: group.quote_items ? validateQuoteItems(group.quote_items, groupIndex) : []
    }
...
function validateQuoteItems(items: any[], groupIndex: number) {
  if (!Array.isArray(items)) {
    throw new ValidationError(`그룹 ${groupIndex + 1}의 품목은 배열이어야 합니다.`, { group_index: groupIndex })
  }
  ...
  return items.map((item, itemIndex) => {
    ...
    const validatedItem = {
      ...
      description: item.description ? sanitizeString(item.description, 1000) : null,
      quantity: validateNumber(item.quantity || 1, '수량', { min: 0 }),
      unit_price: validateNumber(item.unit_price || 0, '단가', { min: 0 }),
      supplier_id: item.supplier_id ? validateUUID(item.supplier_id, '공급업체 ID') : null,
      sort_order: validateNumber(item.sort_order || 0, '정렬 순서', { integer: true, min: 0 }),
      quote_item_details: item.quote_item_details 
        ? validateQuoteItemDetails(item.quote_item_details, groupIndex, itemIndex)
        : []
    }
 ...
 * 견적서 품목 세부사항 검증
 */
function validateQuoteItemDetails(details: any[], groupIndex: number, itemIndex: number) {
  ...
  return details.map((detail, detailIndex) => {
    if (!detail.detail_name) {
      throw new ValidationError(
        ...
        { group_index: groupIndex, item_index: itemIndex, detail_index: detailIndex }
      )
    }

    return {
      detail_name: sanitizeString(detail.detail_name, 200),
      description: detail.description ? sanitizeString(detail.description, 1000) : null,
      quantity: validateNumber(detail.quantity || 1, '수량', { min: 0 }),
      unit_price: validateNumber(detail.unit_price || 0, '단가', { min: 0 }),
      sort_order: validateNumber(detail.sort_order || 0, '정렬 순서', { integer: true, min: 0 })
    }
  })
}
