...
export default function QuoteCalculationSummary({
  ...
  const getSummaryItems = () => {
    ...
    items.push({
      ...
      description:
        formData.vat_type === 'exclusive' ? 'VAT 제외 금액' : 'VAT 포함 금액',
    });
    ...
    <Paper sx={{ p: 3 }}>
              ...
              <Box key={index} sx={{ mb: 2 }}>
                ...
                >
                  ...
                  <Typography
                    ...
                    sx={{
                      fontWeight:
                        item.label.includes('소계') ||
                        item.label.includes('VAT')
                          ? 'bold'
                          : 'normal',
          ...
          <Paper variant='outlined' sx={{ p: 2, backgroundColor: 'grey.50' }}>
            ...
            <Box sx={{ mb: 2 }}>
              ...
              >
                ...
                <Typography variant='body2'>
                  {formatCurrency(calculation.totalCostPrice)}
                </Typography>
              </Box>
              <Divider sx={{ my: 1 }} />
              <Box
      ...
      <Box sx={{ mt: 3, p: 2, backgroundColor: 'primary.50', borderRadius: 1 }}>
        <Typography variant='body2' color='primary.main'>
          📋 견적서 정보: {formData.project_title} | 고객:{' '}
          {formData.customer_name_snapshot} | 발행일: {formData.issue_date} |
          VAT: {formData.vat_type === 'exclusive' ? '별도' : '포함'}
        </Typography>
      </Box>
    </Paper>
  );
