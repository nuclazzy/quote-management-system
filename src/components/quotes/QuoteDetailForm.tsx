...
interface QuoteDetailFormProps {
  ...
  onUpdate: (
    detailIndex: number,
    updates: Partial<QuoteDetailFormData>
  ) => void;
      ...
      <Grid container spacing={2} alignItems='center'>
        ...
        {!detail.is_service && (
          <Grid item xs={6} md={1}>
            <TextField
              ...
              value={detail.days}
              onChange={(e) =>
                handleUpdate({ days: parseFloat(e.target.value) || 0 })
              }
              inputProps={{ min: 0, step: 0.1 }}
            />
          </Grid>
        ...
        <Grid item xs={6} md={1.5}>
          <TextField
            ...
            onChange={(e) =>
              handleUpdate({ unit_price: parseFloat(e.target.value) || 0 })
            }
        ...
        {showCostPrice && (
          <Grid item xs={6} md={1.5}>
            <TextField
              ...
              onChange={(e) =>
                handleUpdate({ cost_price: parseFloat(e.target.value) || 0 })
              }
              inputProps={{ min: 0, step: 1000 }}
        ...
        <Box sx={{ mt: 1, p: 1, backgroundColor: 'grey.50', borderRadius: 1 }}>
          <Typography variant='caption' color='textSecondary'>
            ...
            {(
              ...
              detail.cost_price * detail.quantity
            ).toLocaleString()}{' '}
            | 수익률:{' '}
            {calculateDetailTotal() > 0
              ? (
                  ((calculateDetailTotal() -
                    detail.cost_price * detail.quantity) /
                    calculateDetailTotal()) *
