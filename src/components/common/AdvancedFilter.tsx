...
export default function AdvancedFilter({
  ...
  const handleDateChange = (field: 'date_from' | 'date_to', date: Dayjs | null) => {
    handleFilterChange({
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={3}>
                <Autocomplete
                  ...
                  onChange={(_, newValue) => 
                    handleMultiSelectChange('status', newValue.map(v => v.value as string))
                  }
                  renderInput={(params) => (
                    <TextField {...params} label="상태" placeholder="상태 선택" />
                  )}
              ...
              <Grid item xs={12} md={3}>
                <Autocomplete
                  ...
                  onChange={(_, newValue) => 
                    handleMultiSelectChange('customer_id', newValue.map(v => v.value as string))
                  }
                  renderInput={(params) => (
                    <TextField {...params} label="고객사" placeholder="고객사 선택" />
                  )}
                  renderTags={(value, getTagProps) =>
                    value.map((option, index) => (
                      <Chip
              ...
              <Grid item xs={12} md={3}>
                <Autocomplete
                  ...
                  onChange={(_, newValue) => 
                    handleMultiSelectChange('created_by', newValue.map(v => v.value as string))
                  }
                  renderInput={(params) => (
                    <TextField {...params} label="작성자" placeholder="작성자 선택" />
                  )}
                  renderTags={(value, getTagProps) =>
                    value.map((option, index) => (
                      <Chip
