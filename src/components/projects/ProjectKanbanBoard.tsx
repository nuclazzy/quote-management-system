...
export default function ProjectKanbanBoard({ 
  ...
  const [columns, setColumns] = useState<KanbanColumn[]>([
    { id: 'active', title: '진행중', status: 'active', color: '#2196f3', projects: [] },
    { id: 'on_hold', title: '보류', status: 'on_hold', color: '#ff9800', projects: [] },
    { id: 'completed', title: '완료', status: 'completed', color: '#4caf50', projects: [] },
    { id: 'canceled', title: '취소', status: 'canceled', color: '#f44336', projects: [] }
  ]);
  ...
  const fetchProjects = async () => {
    try {
      ...
      const updatedColumns = columns.map(column => ({
        ...column,
        projects: data?.filter(project => project.status === column.status) || []
      }));
  ...
  const getProjectProfitMargin = (project: Project) => {
    ...
    return ((project.total_revenue - project.total_cost) / project.total_revenue) * 100;
  };
              ...
              <Box p={2}>
                <Box display="flex" alignItems="center" mb={2}>
                  ...
                  <Chip 
                    ...
                    sx={{ ml: 1, backgroundColor: column.color, color: 'white' }}
                  />
                </Box>
                      ...
                      <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                        ...
                        <Box mt={1}>
                          <Typography variant="body2" sx={{ fontSize: '0.75rem' }}>
                            매출: <strong>{project.total_revenue.toLocaleString()}원</strong>
                          </Typography>
                          <Typography variant="body2" sx={{ fontSize: '0.75rem' }}>
                            원가: <strong>{project.total_cost.toLocaleString()}원</strong>
                          </Typography>
                          <Typography 
                            variant="body2" 
                            sx={{ 
                              fontSize: '0.75rem',
                              color: getProjectProfitColor(getProjectProfitMargin(project))
                            }}
                          >
                            수익률: <strong>{getProjectProfitMargin(project).toFixed(1)}%</strong>
                          </Typography>
                        </Box>
                        ...
                        <Box display="flex" gap={0.5} mt={1} onClick={(e) => e.stopPropagation()}>
                          {columns.filter(col => col.status !== project.status).map((col) => (
                            <Button
                              key={col.id}
                              size="small"
                              variant="outlined"
